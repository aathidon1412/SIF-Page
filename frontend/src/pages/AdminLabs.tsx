              ...
              return (
                <div key={item.id} className={`bg-gray-50 rounded-xl border-2 border-gray-100 overflow-hidden hover:shadow-lg transition-all duration-300 ${!available ? 'opacity-60' : ''}`}>
                  <div className="relative h-40 bg-gradient-to-br from-blue--100 to-blue--50">
                    {(item as any).image ? (<img src={(item as any).image} alt={(item as any).name} className="w-full h-full object-cover" />) : (<div className="w-full h-full flex items-center justify-center"><svg className="w-16 h-16 text-blue--300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg></div>)}
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between mb-3 pb-3 border-b border-gray-200"><span className="text-xl font-bold text-blue-950">${(item as any).pricePerHour}/hr</span><span className="text-xs text-gray-500">Capacity: {(item as any).capacity}</span></div>
